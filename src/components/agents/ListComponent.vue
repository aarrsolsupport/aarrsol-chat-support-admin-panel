            ...
            <div class="search-sec" v-if="show_search">
                ...
                <div class="operator-btn">
                    ...
                    <button class="thm-btn" data-bs-toggle="offcanvas" data-bs-target="#agentoffcanvas"
                        aria-controls="agentoffcanvas" @click="setAgForm(1, {})"> Add Agent </button>
                </div>
            </div>
                        ...
                        <tbody>
                            <tr v-for="(item, index) in filteredItems" :key="index">
                                ...
                                <td class="website-link">
                                    <router-link :to="'/agent-details/' + item.id">
                                        <h2>{{ item.name }}</h2>
                                    </router-link>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="Categories-btn">
                                        <button class="thm-btn grey-bg" v-for="(cat, index) in item.categories"
                                            :key="index">{{ cat.description }}</button>
                                    </div>
                                </td>
                                <td>
                                    <div class="operator-check-con">
                                        <div class="form-check form-switch">
                                            <input class="form-check-input" type="checkbox"
                                                :id="'flexSwitchCheckDefault.' + item.id" :checked="item.is_active ?? null"
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end more-action-list">
                                            <li>
                                                <button class="dropdown-item more-list-btn" type="button"
                                                    data-bs-toggle="offcanvas" data-bs-target="#agentoffcanvas"
                                                    aria-controls="agentoffcanvas" @click="setAgForm(2, item)">
                                                    <div class="edit-icon"><img src="@/assets/images/edit-icon.svg" alt="">
                                                    </div>
                                            ...
                                            <li>
                                                <button class="dropdown-item more-list-btn" type="button"
                                                    data-bs-toggle="offcanvas" data-bs-target="#agentoffcanvas"
                                                    aria-controls="agentoffcanvas" @click="setAgForm(3, item)">
                                                    <div class="edit-icon"><img src="@/assets/images/update-password.svg"
                                                            alt=""></div>
                                                    <div class="thm-heading">
        ...
        '$store.state.agent_change': function () {
            for (var i = 0; i < this.listItems.length; i++) {
                if (this.listItems[i].id == this.$store.state.agent_change.item.id && this.$store.state.agent_change.item.form_type == 2) {
                    ...
                    this.listItems[i].categories = Object.assign({}, this.$store.state.agent_change.item.categories)
